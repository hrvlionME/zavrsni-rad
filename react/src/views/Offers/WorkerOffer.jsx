...
function WorkerOffer() {
  ...
  const fetchIsFollowing = async () => {
    try {
      const response = await axiosClient.get(`/isfollowing/${id}`);
  ...
  const addToFollowings = () =>{
    axiosClient
      .post(`/followoffer/${id}`)
      .then()
      .catch((error) => {
        console.error("Error adding offer to followings:", error);
      });
      fetchIsFollowing();
  }
          ...
          >
            ...
            <div className="lg:relative w-full lg:w-2/6 flex flex-col lg:items-end lg:gap-y-8">
              <div className="flex justify-center lg:justify-end">
                <button className="text-white bg-secondary-base px-4 lg:px-6 py-2 rounded-md font-open text-xl my-2 lg:my-10 mx-2 lg:mx-5" onClick={addToFollowings}>
                  {following ? "Otprati" : "Zaprati"}
                </button>
                <Link to={`/offers/${id}/apply`} className="text-white bg-secondary-base px-4 lg:px-6 py-2 rounded-md font-open text-xl my-2 lg:my-10 mx-2 lg:mx-5">
                  Prijavi se
                </Link>
              </div>
